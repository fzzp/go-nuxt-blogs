...
type Metadata struct {
	CurrentPage  int `json:"currentPage,omitempty"`  // 当前是第几页
	PageSize     int `json:"pageSize,omitempty"`     // 每页多少条
	FirstPage    int `json:"firstPage,omitempty"`    // 起始页码 -> 1
	LastPage     int `json:"lastPage,omitempty"`     // 最后一页
	TotalRecords int `json:"totalRecords,omitempty"` // 总记录条数
}
